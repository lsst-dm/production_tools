...
butler_cache = {}
if os.getenv("BUTLER_REPO_NAMES") is None:
    print("BUTLER_REPO_NAMES was not set, images endpoint will not work")
else:
    for repo in os.getenv("BUTLER_REPO_NAMES").split(","):
        butler_cache[repo] = Butler(repo)
...
def dataId(repo, collection, plot_name, data_id):
    ...
    if repo_decoded not in butler_cache.keys():
        return {'error': 'Invalid repo'}, 400


    butler = butler_cache[repo_decoded]
...
def index(repo, uuid):
    ...
    if repo not in butler_cache.keys():
        return {'error': 'Invalid repo'}, 400

    butler = butler_cache[repo]
